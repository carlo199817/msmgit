...
class Sales(models.Model):
    ...
    author = models.ForeignKey(User, related_name='sales', on_delete=models.CASCADE)  # foreign key calls on an outside model whether imported or in this file, CASCADE will delete the post if
    def __str__(self):
        return self.product+"   "+self.quantity +" "+self.price

class SalesTotal(models.Model):
    total = models.CharField(max_length=200, blank=True)
    timenow = models.CharField(max_length=200, blank=True)
    datenow = models.CharField(max_length=200, blank=True)    # caption is a field in this Post model. it specifies a class attribute Charfield and represents a database column. blank=True lets the field be optional left empty
    date_posted = models.DateTimeField(default=timezone.now)  # instead of hard setting the time this timezone.now takes the users timezone into consideration.
    date_updated = models.DateTimeField('date_updated', auto_now=True)
    author = models.ForeignKey(User, related_name='salestotal', on_delete=models.CASCADE)  # foreign key calls on an outside model whether imported or in this file, CASCADE will delete the post if
    def __str__(self):
        return self.total
